...
CustomGenerator::CustomGenerator(string filename) :
	filename(filename)
{
...
void CustomGenerator::generateTypesFile(string funct_name, map<string, string> param_type,
...
void CustomGenerator::generateTestCasesFile(string funct_name, map<string, string> param_type,
	vector<string> insert_order, string return_type)
{
	if(testcases_file.is_open())
	{
...
//%-------------------------------------------------------------------------------
/*
bool CustomReader::generateExecutableTests(string class_test, string types_path, string values_path, string output_path)
{
		...
		if (types_file.is_open())
		{
			while(getline(types_file, line))
			{
				...
				} else
				{
					...
					method_types.insert(pair<string, vector<string>(method_name, types));
					types.clear();
				}
			}
		}
		...
		if (file.is_open())
		{
			while(getline(file, line))
			{
				...
				} else
				{
					...
					method_values.insert(pair<string, vector<string>(method_name, values));
					values.clear();
				}
			}
		}
