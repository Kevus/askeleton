...
// Additional includes (only appears if needed)
{includes}

// Additional namespaces (only appears if needed)
{namespaces}
using namespace std;

struct Fixture {
    Fixture() : argc(Catch::Session().argc()), argv(Catch::Session().argv()) {
        stringstream file;
        file << argv[1];
    ...
    bool getConfigParameters(string cfgPath) {
        ...
        if (configFile.is_open()) {
            ...
            while (getline(configFile, line)) {
                ...
                // If we reach this point, then we can store the values in the intermediate state
